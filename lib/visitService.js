...
export async function createDeferredVisit(visitData, date, warnings) {
  ...
  try {
    ...
    if (dErr) {
      if (Array.isArray(warnings)) warnings.push(`Failed to create deferred visit for recurrence starting ${date}`)
      return null
    }
    if (Array.isArray(warnings)) warnings.push(`Deferred visit created (id=${dData.id}) with earliestDate=${date}`)
    return dData
  } catch (e) {
    if (Array.isArray(warnings)) warnings.push(`Failed to create deferred visit for recurrence starting ${date}`)
    return null
