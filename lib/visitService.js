export async function updateVisitTableDate(visitId, originalDate, newDate) {
  const { error } = await supabase
    .from('visits')
    .update({ visitDate: newDate })
    .match({ id: visitId, visitDate: originalDate });
...
export async function listVisits({ fromDate, limit = 25, order = 'asc' } = {}) {
  let q = supabase
    .from('calendars')
    .select('id, date, visitId, visits!inner(addressId, visitDate, time, notes, isInside, recurrence, visitTypeId)');
...
export async function listUnscheduledVisits() {
  const { data, error } = await supabase
    .from('visits')
    .select('id, addressId, visitDate, time, notes, isInside, recurrence, earliestDate, visitTypeId')
    .is('visitDate', null);
...
export async function createDeferredVisit(visitData, date, warnings) {
  const deferred = {
    ...
    visitTypeId: (visitData.visitTypeId === '' || visitData.visitTypeId == null) ? null : Number(visitData.visitTypeId),
  };
  try {
...
export async function updateFutureVisitTableDates(visitId, daysDiff, originalDate) {
  ...
  if (error) throw error;
  if (visitRow) {
    const oldDate = new Date(visitRow.visitDate);
    oldDate.setDate(oldDate.getDate() + daysDiff);
