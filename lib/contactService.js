...
export async function getContact(contactId) {
  const { data, error } = await supabase
    .from('contacts')
    .select('*, facebookglennmetadata, facebookhandymanmetadata, addresses(id, address, visits(id, visitDate, earliestDate, notes, isInside, isFlexilbe, time, recurrence, visitTypeId))')
    .eq('id', contactId)
    .single();
