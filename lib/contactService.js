...
async function upsertVisit(v, addressId) {
  if (v && v.id) {
    const dbRow = { addressId, visitDate: v.visitDate || null, notes: v.notes, isInside: v.isInside, isFlexilbe: v.isFlexilbe, time: v.time, recurrence: v.recurrence || 'does not reoccur' }
    ...
    if (data && data.visitDate) await saveVisitToCalendar(data)
    return data
  }
...
async function insertAddressesAndVisits(contactId, addresses = [], results) {
  for (const addr of addresses || []) {
    ...
    for (const v of addr.visits || []) {
      const vData = await upsertVisit(v, aData.id)
