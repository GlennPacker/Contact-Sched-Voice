...
async function upsertAddress(contactId, addr) {
  if (addr && addr.id) {
    const { data, error } = await supabase.from('addresses').update({ address: addr.address }).eq('id', addr.id).select('*').single()
...
async function processRecurrence(visitData, warnings) {
  ...
  for (const date of dates) {
    const ok = await dailyHoursValid({ visitDate: date, time: visitData.time }, visitData.id)
    if (!ok) {
      warnings.push(`Recurrence for visit on ${visitData.visitDate} stopped at ${date}`)
    ...
    try {
      await saveVisitToCalendar({ id: visitData.id, visitDate: date, time: visitData.time })
    } catch (e) {
      warnings.push(`Failed to add recurrence for visit on ${visitData.visitDate} at ${date}: ${e && e.message ? e.message : e}`)
      continue
    }
  }
}
...
async function upsertVisit(v, addressId, results) {
  if (v && v.id) {
    const dbRow = { addressId, visitDate: v.visitDate || null, notes: v.notes, isInside: v.isInside, isFlexilbe: v.isFlexilbe, time: v.time, recurrence: v.recurrence || 'does not reoccur', earliestDate: v.earliestDate || null }
    ...
    // process recurrence dates and collect warnings
    if (results && Array.isArray(results.warnings)) await processRecurrence(data, results.warnings)
    return data
  }
...
async function insertAddressesAndVisits(contactId, addresses = [], results) {
  for (const addr of addresses || []) {
    ...
    // If this is an existing address, remove any visits that were deleted in the incoming payload
    if (aData && aData.id) {
      ...
      const incomingIds = (addr.visits || []).map(v => v.id).filter(Boolean)
...
export async function getContact(contactId) {
  const { data, error } = await supabase
    .from('contacts')
    .select('*, addresses(id, address, visits(id, visitDate, earliestDate, notes, isInside, isFlexilbe, time, recurrence))')
    .eq('id', contactId)
    .single()
...
export async function updateContact(contactId, contact) {
  ...
  const incomingAddrIds = (contact.addresses || []).map(a => a.id).filter(Boolean)
