    ...
    jest.doMock('./supabaseClient', () => ({
      supabase: {
        from: table => {
          ...
          if (table === 'addresses') {
            return { select: () => ({ eq: () => Promise.resolve({ data: existingAddrs, error: null }) }), delete: () => ({ in: (field, ids) => { calls.deletedAddrs = ids; return Promise.resolve({ data: null, error: null }) } }) }
          }
          if (table === 'visits') {
            return {
              select: () => ({ in: (field, ids) => Promise.resolve({ data: visitsToDelete, error: null }) }),
              delete: () => ({ in: (field, ids) => { calls.deletedVisits = ids; return Promise.resolve({ data: null, error: null }) } })
            }
          }
          if (table === 'calendars') {
            return { delete: () => ({ in: (field, ids) => { calls.deletedCalendars = ids; return Promise.resolve({ data: null, error: null }) } }) }
          }
          return {
