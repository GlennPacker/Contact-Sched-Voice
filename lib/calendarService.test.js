...
describe('updateFutureCalendarDates', () => {
  it('updates future calendar dates for a visitId', async () => {
    const mockRows = [
      { id: 1, visitId: 1, date: '2026-01-13' },
      { id: 2, visitId: 1, date: '2026-01-14' }
    ];
    const updateCalendarDate = jest.fn(() => Promise.resolve());
    jest.spyOn(require('./calendarService.js'), 'listAllByVisitId').mockResolvedValue(mockRows);
    jest.spyOn(require('./calendarService.js'), 'updateCalendarDate').mockImplementation(updateCalendarDate);
    await updateFutureCalendarDates(1, 2, '2026-01-13');
    expect(updateCalendarDate).toHaveBeenCalledTimes(2);
  });
});
