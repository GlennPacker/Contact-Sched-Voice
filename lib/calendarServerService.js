...
export async function getCalendarEventsSSR({ month }) {
  ...
  console.log('Calendar query - fromDate:', fromDate);
  console.log('All visits count:', allVisits?.length);
  console.log('All visits:', allVisits?.map(v => ({ id: v.id, visitDate: v.visitDate, time: v.time })));
  ...
  const events = (allVisits || []).map(v => {
    ...
    const start = new Date(v.visitDate + 'T09:00:00');
    const end = new Date(start);
    end.setHours(end.getHours() + (HOURS[v.time] || 2));
  ...
  console.log('Final events:', events?.map(e => ({ id: e.id, title: e.title, start: e.start, end: e.end })));
  return events;
}
