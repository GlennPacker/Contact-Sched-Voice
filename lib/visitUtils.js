...
export function getVisitEventColor(v) {
...
export function getEndDate(time, days, startUTC) {
  ...
  switch (time) {
    case '1/2 day':
      endUTC.setUTCMinutes(endUTC.getUTCMinutes() + 210);
      break;
    case '2 hours':
      endUTC.setUTCHours(endUTC.getUTCHours() + 2);
      break;
    case 'multiday':
      endUTC.setUTCDate(endUTC.getUTCDate() + (+days || 0));
    case 'multiday':
    case 'full day':
...
export function contactCreateAppointment({ contactName, visit, address }, createAppointmentFn = createAppointment) {
  if (!visit || !visit.visitDate || !visit.time) return '';
  const startUTC = calcStart(...visit.visitDate.split('-'), visit.time);

  return createAppointmentFn({
    title: contactName,
    details: visit.notes,
    location: address,
    start: startUTC,
